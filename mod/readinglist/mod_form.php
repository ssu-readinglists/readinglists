...
/**
 * Reading list configuration form
 *
 * @package    mod
...
class mod_readinglist_mod_form extends moodleform_mod {
    function definition() {
        ...
        if (count($options) == 1) {
            ...
            $mform->addElement('select', 'display', get_string('displayselect', 'readinglist'), $options);
            $mform->setDefault('display', $config->display);
            $mform->setAdvanced('display', $config->display_adv);
        }

        if (array_key_exists(RESOURCELIB_DISPLAY_POPUP, $options)) {
            $mform->addElement('text', 'popupwidth', get_string('popupwidth', 'readinglist'), array('size'=>3));
            if (count($options) > 1) {
            ...
            $mform->addElement('text', 'popupheight', get_string('popupheight', 'readinglist'), array('size'=>3));
            if (count($options) > 1) {
        ...
        $mform->addElement('advcheckbox', 'printheading', get_string('printheading', 'readinglist'));
        $mform->setDefault('printheading', $config->printheading);
        $mform->setAdvanced('printintro', $config->printheading_adv);
        $mform->addElement('advcheckbox', 'printintro', get_string('printintro', 'readinglist'));
    ...
    function data_preprocessing(&$default_values) {
        if ($this->current->instance) {
            $draftitemid = file_get_submitted_draft_itemid('readinglist');
            $default_values['readinglist']['format'] = $default_values['contentformat'];
            $default_values['readinglist']['text']   = file_prepare_draft_area($draftitemid, $this->context->id, 'mod_readinglist', 'content', 0, readinglist_get_editor_options($this->context), $default_values['content']);
            $default_values['readinglist']['itemid'] = $draftitemid;
        }
