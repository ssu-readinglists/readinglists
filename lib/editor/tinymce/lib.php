...
//ADDED CODE
		global $USER;
		if (has_capability('mod/readinglist:edit',get_context_instance(CONTEXT_SYSTEM))){//IF USER DOES NOT HAVE THIS CAPABILITY DO NOT SHOW THE BLOCK
			$refshare = ',refshare';
		    }else{
			$refshare = '';
	        }
...
//AMENDMENT
//ADDED CODE
        if (strpos($_SERVER['REQUEST_URI'],'bui_editid') !== false) {// ARE WE IN A BLOCK?
		$libvids = '';
        }else{
		$libvids = ',|,libvids';	
		}
...
//END REMOVED CODE
//ADDED CODE
                    'plugins' => "{$xmedia}advimage,safari,table,style,layer,advhr,advlink,emotions,inlinepopups,searchreplace,paste,directionality,fullscreen,moodlenolink,{$xemoticon}{$xdragmath}nonbreaking,contextmenu,insertdatetime,save,iespell,preview,print,noneditable,visualchars,xhtmlxtras,template,pagebreak,spellchecker{$refshare},webcam,libvids",
//END ADDED CODE
//END AMENDMENT		
...
//END REMOVED CODE
//ADDED CODE
                    'theme_advanced_buttons3_add' => "table, hr,|,code,{$spellbutton}{$refshare},|,webcam {$libvids}",	
//END ADDED CODE
//END AMENDMENT	
